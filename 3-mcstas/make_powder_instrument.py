...
def add_guide(instrument, source):
    ...
    for index in range(n_segments):
        ...
        guide.set_ROTATED([0, f"guide_curve_deg/{n_segments:.1f}", 0], RELATIVE=previous_component)
...
def add_backend_union(instrument, include_event_monitors=True):
    ...
    for index, angle in enumerate(tube_angles):
        ...
        casing.set_parameters(radius=0.05, yheight="detector_height", material_string='"Al"',
                              priority=10 + index)
...
def make(union_detectors=True, include_choppers=True, include_event_monitors=True, **kwargs):
