...
/* token modifiers */
	...
	// Add code to find the index of pixel column
	int var_index;
	for (var_index=0;var_index<30;var_index++) {
		if (strcmp(Vars->Coord_Var[var_index], "id") == 0) break;
	}
	long pix=Vars->Coord_Min[var_index];
	// This did not work, pixel min was not found for some reason
...
int Monitor_nD_Trace(MonitornD_Defines_type *DEFS, MonitornD_Variables_type *Vars, _class_particle* _particle)
{
  ...
  if ((Vars->Buffer_Counter >= Vars->Buffer_Block) && (Vars->Flag_List >= 2))
  {
    ...
    {
      Vars->Mon2D_Buffer  = (double *)realloc(Vars->Mon2D_Buffer, (Vars->Coord_Number+1)*(Vars->Buffer_Counter+Vars->Buffer_Block)*sizeof(double));
      if (Vars->Mon2D_Buffer == NULL)
            { printf("Monitor_nD: %s cannot reallocate Vars->Mon2D_Buffer[%li] (%li). Skipping.\n", Vars->compcurname, i, (long int)(Vars->Buffer_Counter+Vars->Buffer_Block)*sizeof(double)); Vars->Flag_List = 1; }
      else {
		  // Successfully reallocated, note new size and start over
		  Vars->Buffer_Block = Vars->Buffer_Counter+Vars->Buffer_Block;
		  Vars->Buffer_Size = Vars->Buffer_Block;
		  //Vars->Buffer_Counter = 0; // Continue where we were, resetting it is bad
	  }
    }
