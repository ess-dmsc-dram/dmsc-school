...
def apply_random_noise(
    arr: np.ndarray, noise_scale: float = 1.0, *, rng: np.random.Generator
) -> np.ndarray:
...
def _heart_signal(rng: np.random.Generator) -> sc.DataArray:
    ...
    noise_x, noise_y = (
        apply_random_noise(coord, noise_scale=4, rng=rng) for coord in (raw_x, raw_y)
    )
...
def circle(
    ...
    return np.cos(param_t) * c_spec.r + c_spec.cx, np.sin(
        param_t
    ) * c_spec.r + c_spec.cy


def tear_drop(
    rng: np.random.Generator, size: int, eye_spec: CircleSpec
) -> tuple[np.ndarray, np.ndarray]:
    width = eye_spec.r / 20
    height = width * 2
    cx = eye_spec.cx
...
def _smiley_signal(rng: np.random.Generator) -> sc.DataArray:
    ...
    parts = tuple(
        circle(rng, size=part_size, c_spec=part_spec)
