...
def make(**kwargs):
    ...
    # Calculations
    detector_offset = instrument.add_declare_var("double", "detector_offset", value=0.25)
    analyzer_angle = instrument.add_declare_var("double", "analyzer_angle")
    instrument.append_initialize("analyzer_angle = RAD2DEG*0.5*atan(detector_offset/analyzer_distance);")
    instrument.append_initialize('printf("Analyzer_angle: %lf \\n", analyzer_angle);')
    
    instrument.add_declare_var("double", "backscattering_wavelength")
    instrument.append_initialize("backscattering_wavelength = 2*analyzer_d*sin(DEG2RAD*0.5*(180-2.0*analyzer_angle));")
    instrument.add_declare_var("double", "backscattering_energy")
    instrument.append_initialize("backscattering_energy = (2.0*PI/backscattering_wavelength)*K2V*(2.0*PI/backscattering_wavelength)*K2V*VS2E;")
    
    instrument.append_initialize('printf("2 theta: %lf \\n", DEG2RAD*0.5*(180-2.0*analyzer_angle));')
    instrument.append_initialize('printf("Selected wavelength: %lf \\n", 2*analyzer_d*sin(DEG2RAD*0.5*(180-2.0*analyzer_angle)));')
    
    
    instrument.add_parameter("double", "energy_width_ueV", comment="Simulated energy range in micro eV")
    ...
    instrument.append_initialize("min_energy = backscattering_energy - 0.5*energy_width_meV;")
    instrument.append_initialize("max_energy = backscattering_energy + 0.5*energy_width_meV;")
    ...
    instrument.add_parameter("double", "n_pulses", value=3.0, comment="Number of pulses from source")
    src.acc_power = "2*(n_pulses/14)"
    src.n_pulses = "n_pulses"
    src.append_EXTEND("// Compensate for lack of guide and analyzer area with weight increase")
    src.append_EXTEND("p*=1500*1E5;")
    ...
    instrument.append_initialize("t_min_sample = (min_wavelength)*(sample_distance - 0.05)/(K2V*2*PI);")
    instrument.append_initialize("t_max_sample = (max_wavelength)*(sample_distance + 0.2)/(K2V*2*PI);")
    instrument.append_initialize("t_max_sample_pulses = t_max_sample + 3.0E-3; // Account for ESS pulse structure")
    ...
    sample_choice = instrument.add_parameter("string", "sample_choice", value='"Elastic"',
                                             comment="Choice of sample type")
    ...
    # Known quasi
    instrument.add_parameter("double", "gamma_ueV", value=10, comment="[ueV] Energy width of known quasi-elastic sample")
    ...
    material = instrument.add_component("known_quasi_elastic_material", "Union_make_material")
    ...
    material = instrument.add_component("unknown_quasi_elastic_material", "Union_make_material")
    material.set_parameters(my_absorption=0.57, process_string='"quasi_incoherent_1,quasi_incoherent_2"')


    analyzer_direction =  instrument.add_declare_var("double", "analyzer_direction", value=30)
    dist = instrument.add_parameter("double", "analyzer_distance", value=3.0,
                                    comment="[m] Sample analyzer distance")

    all_samples = dict(radius=0.01, yheight=0.03, p_interact=0.4,
                       target_z=dist, focus_xw=0.01, focus_xh=0.01)

    cylinder = instrument.add_component("elastic_sample", "Union_cylinder")
    cylinder.set_parameters(**all_samples,
                            material_string='"elastic_material"', priority=10,
                            number_of_activations="enable_elastic", visualize="enable_elastic")
    ...
    cylinder.set_parameters(**all_samples,
                            ...
                            number_of_activations="enable_known_quasi", visualize="enable_known_quasi")
    cylinder.set_AT(0, RELATIVE=sample_position)
    cylinder.set_ROTATED([0, analyzer_direction, 0], RELATIVE=sample_position)

    cylinder = instrument.add_component("unknown_quasi_elastic_sample", "Union_cylinder")
    cylinder.set_parameters(**all_samples,
                            material_string='"unknown_quasi_elastic_material"', priority=12,
                            number_of_activations="enable_unknown_quasi", visualize="enable_unknown_quasi")
    cylinder.set_AT(0, RELATIVE=sample_position)
    cylinder.set_ROTATED([0, analyzer_direction, 0], RELATIVE=sample_position)
    ...
    analyzer_q = instrument.add_declare_var("double", "analyzer_q", value=2*np.pi/3.135)

    analyzer = instrument.add_component("analyzer", "Monochromator_flat")
    analyzer.set_parameters(zwidth=0.05, yheight=0.05,
                            r0=0.7, Q=analyzer_q, mosaich=30, mosaicv=30)
    ...
    casing.set_parameters(yheight=0.1, radius=0.03, p_interact=0.25,
    ...
    He3_gas.set_parameters(yheight=casing.yheight-0.02, radius=casing.radius-4E-3,
                           material_string='"He3"', priority=310, p_interact=0.2)
    ...
    instrument.append_initialize("t_min = (backscattering_wavelength)*(sample_distance - 0.1 + 2*analyzer_distance)/(K2V*2*PI);")
    instrument.append_initialize("t_max = (backscattering_wavelength)*(sample_distance + 1.0 + 2*analyzer_distance)/(K2V*2*PI);")
    instrument.append_initialize("t_max = t_max + 4.0E-3; // Account for ESS pulse structure")
    instrument.append_initialize("t_max_pulses = t_max + (n_pulses-1.0)*1.0/14.0; // Account for n_pulses")

    
    detector = instrument.add_component("signal_space", "Union_abs_logger_1D_space", RELATIVE=He3_gas)
    detector.target_geometry = '"He3_gas"'
    detector.yheight = He3_gas.yheight
    detector.n = 50
    detector.filename = '"detector_signal_space.dat"'
    
    detector = instrument.add_component("signal_time", "Union_abs_logger_1D_time", RELATIVE=He3_gas)
    detector.target_geometry = '"He3_gas"'
    ...
    detector = instrument.add_component("signal_tof_all", "Union_abs_logger_1D_space_tof",
                                        RELATIVE=He3_gas)
    detector.target_geometry = '"He3_gas"'
    detector.yheight = He3_gas.yheight
    detector.n = 50
...
def add_chopper_code(instrument):
    ...
    instrument.add_parameter("double", "frequency_multiplier", value=3, comment="Ratio between chopper and source frequency")
    instrument.add_declare_var("double", "chopper_distance", value=6.5, comment="[m] Distance from source to chopper")

    instrument.add_declare_var("double", "chopper_delay")
    instrument.append_initialize("chopper_delay = 0.5*2.86E-3 + backscattering_wavelength*chopper_distance/(K2V*2*PI);")
    ...
    instrument.append_initialize("chopper_time_width = (chopper_theta/360)/chopper_frequency;")
